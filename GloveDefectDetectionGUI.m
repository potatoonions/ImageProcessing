...
% ===== LEFT CONTROL PANEL =====
leftPanel = uipanel(fig, 'Position', [0.01, 0.1, 0.18, 0.88], ...
    'Title', 'Control Panel', 'FontSize', 11, 'BackgroundColor', [0.96 0.96 0.96], ...
    'Units', 'pixels');

% Convert panel position to pixels for consistent child positioning
panelHeightPixels = 0.88 * 900;  % Panel height in pixels

% Upload Button - Browse Files
uploadBtn = uicontrol(leftPanel, 'Style', 'pushbutton', ...
    'String', 'Browse & Upload', ...
    'Position', [10, panelHeightPixels - 60, 130, 50], ...
...
uicontrol(leftPanel, 'Style', 'text', 'String', 'Material Type:', ...
    'Position', [10, panelHeightPixels - 100, 130, 20], 'FontSize', 10, 'FontWeight', 'bold', ...
    'BackgroundColor', [0.96 0.96 0.96]);

% Cloth Gloves (Only option)
uicontrol(leftPanel, 'Style', 'text', 'String', 'Cloth Gloves', ...
    'Position', [20, panelHeightPixels - 135, 120, 25], 'FontSize', 11, 'FontWeight', 'bold', ...
...
appData.processBtn = uicontrol(leftPanel, 'Style', 'pushbutton', ...
    ...
    'Position', [10, panelHeightPixels - 215, 130, 50], ...
    'FontSize', 10, 'FontWeight', 'bold', ...
...
uicontrol(leftPanel, 'Style', 'pushbutton', ...
    'String', 'Clear All', ...
    'Position', [10, panelHeightPixels - 280, 130, 45], ...
    'FontSize', 10, 'FontWeight', 'bold', ...
...
appData.statusLabel = uicontrol(leftPanel, 'Style', 'text', ...
    'String', 'Ready', ...
    'Position', [10, panelHeightPixels - 370, 130, 70], ...
...
set(appData.originalAx, 'Title', text(0.5, 1.05, 'Original Glove Image', ...
    'Parent', appData.originalAx, 'HorizontalAlignment', 'center'));

% Tab 2: Processing Pipeline
tab2 = uitab(tabGroup, 'Title', 'Processing Pipeline');

appData.grayAx = subplot(2, 2, 1, 'Parent', tab2);
set(appData.grayAx, 'Title', 'Step 1: Grayscale Conversion', 'FontSize', 10);
axis(appData.grayAx, 'image');

appData.maskAx = subplot(2, 2, 2, 'Parent', tab2);
set(appData.maskAx, 'Title', 'Step 2: Raw Color Mask', 'FontSize', 10);
axis(appData.maskAx, 'image');

appData.morphAx = subplot(2, 2, 3, 'Parent', tab2);
set(appData.morphAx, 'Title', 'Step 3: After Morphology', 'FontSize', 10);
axis(appData.morphAx, 'image');

appData.finalAx = subplot(2, 2, 4, 'Parent', tab2);
set(appData.finalAx, 'Title', 'Step 4: Final Result', 'FontSize', 10);
...
uicontrol(tab3, 'Style', 'text', 'String', 'Detected Material:', ...
    'Position', [20, 750, 200, 30], 'FontSize', 12, 'FontWeight', 'bold', ...
...
appData.materialLabel = uicontrol(tab3, 'Style', 'text', 'String', 'Not Analyzed', ...
    'Position', [250, 750, 300, 30], 'FontSize', 14, 'FontWeight', 'bold', ...
    'ForegroundColor', [0.2 0.4 0.6], 'BackgroundColor', [0.96 0.96 0.96]);

uicontrol(tab3, 'Style', 'text', 'String', 'Detected Defect:', ...
    'Position', [20, 700, 200, 30], 'FontSize', 12, 'FontWeight', 'bold', ...
    'HorizontalAlignment', 'left', 'BackgroundColor', [0.96 0.96 0.96]);

appData.defectLabel = uicontrol(tab3, 'Style', 'text', 'String', 'Not Analyzed', ...
    'Position', [250, 700, 300, 30], 'FontSize', 14, 'FontWeight', 'bold', ...
    'ForegroundColor', [0.2 0.4 0.6], 'BackgroundColor', [0.96 0.96 0.96]);

appData.defectCountLabel = uicontrol(tab3, 'Style', 'text', ...
    'String', 'Total Defects Found: 0', ...
    'Position', [20, 640, 500, 25], 'FontSize', 11, ...
    'BackgroundColor', [0.96 0.96 0.96]);

% Metrics Table
appData.metricsTable = uitable(tab3, 'Position', [20 50 750 570], ...
...
% ===== RIGHT INFO PANEL =====
rightPanel = uipanel(fig, 'Position', [0.90, 0.1, 0.09, 0.88], ...
...
% ===== HEADER =====
headerPanel = uipanel(fig, 'Position', [0, 0.95, 1, 0.05], ...
