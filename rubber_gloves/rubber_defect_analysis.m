function rubber_defect_analysis()
    ...
    DEFECT_TYPES = ["Hole", "Stain", "Normal"];
        ...
        for i = 1:numel(imgs)
            ...
            switch char(defectType)
                case 'Hole'
                    ...
                    expectedDetector = 'Hole';
                case 'Stain'
                    overlay = rubberTearDetection(img);
                    expectedDetector = 'Stain';
                case 'Normal'
                    overlay = zeros(size(originalImg, 1), size(originalImg, 2), 4, 'uint8');
                    expectedDetector = 'Normal';
            end
            
            if strcmpi(char(defectType), 'Normal')
